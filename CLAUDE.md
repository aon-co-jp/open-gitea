...
- **2026-08-01 電源/機能プロファイル(チェックボックス+ボタン)を追加+
  実バグ発見・修正: コミット済み`static/`バンドルが2026-07-27の
  `RGit`→`open-gitea`リネーム以降ずっと再生成されていなかった**:
  1. **電源プロファイルUI**: `open-raid-z/CLAUDE.md`のエコシステム標準
     方針への対応。ユーザー指示で「省メモリ・常時電源接続などは
     チェックボックスとボタンに」という形式指定を受け、
     `open-easy-web`の`PowerProfileFlags`(省電力/省メモリ/常時電源接続を
     独立チェックボックスとして組み合わせ可能、「通常」は3つとも未
     チェック)と同じ設計をチェックボックスUIとして移植。「省機能表示に
     切替」「全機能を復元」はボタンのまま(open-redmine先行実装と
     同じ、UIの見せ方が異なるだけでチェックボックス化していない)。
     **正直な開示**: このアプリにもバックグラウンドポーリングループが
     無いため、実際に効果があるのは1軸のみ——省電力または省メモリの
     いずれかが有効ならリポジトリ選択時のWikiページ一覧自動取得を止め、
     常時電源接続が有効ならその抑制を上書きして常に自動取得する
     (`open-easy-web`の「常時電源接続がバッテリー節約軸を上書きする」
     という合成ルールと同じ考え方)。省電力チェックボックス自体は今は
     単独では効果を持たないが(省メモリと同じ抑制軸に相乗り)、将来
     ポーリング処理を追加する際にすぐ使えるようUI規約として先行して
     用意した。
  2. **実バグ発見・修正**: `git ls-files static/`で確認したところ、
     コミット済みの`wasm-bindgen`生成物が`static/rgit_web.js`
     (2026-07-27のリネーム前の名前)のままで、`index.html`が実際に
     importする`open_gitea_web.js`は**リポジトリに一度も追加されて
     いなかった**——`grep -c wiki static/rgit_web.js`が0件だったことから、
     このコミット済みバンドルはWiki機能実装(2026-07-22)より前の、
     半年近く古いビルドだったと判明。本番がこれまで動いていたのは、
     VPS上でローカルに再ビルドした(未コミットの)`open_gitea_web.*`が
     残っていたためと推測される(正直な開示: このセッションでは
     VPS側の実際の状態は未確認)。`git rm`で`rgit_web.*`一式を削除し、
     新規ビルドした`open_gitea_web.js`/`open_gitea_web_bg.wasm`を
     コミットして解消。
  3. **検証**: `cargo test`(サーバー本体)31件+RPoem互換試用1件、
     全green(回帰無し)。`cargo build --target wasm32-unknown-unknown
     --release`警告0件。**ローカルで実際にブラウザ操作で確認**
     (静的ファイルを`python -m http.server`で配信、バックエンド無しで
     チェックボックス/ボタンの純粋なDOM挙動を検証): (a) 「省メモリ」
     チェック→ステータス文言が「有効: 省メモリ」に変化・
     `localStorage`へ`1`永続化、(b) 追加で「常時電源接続」をチェック→
     「有効: 省メモリ + 常時電源接続」に更新、(c) 「省機能表示に切替」→
     `#wiki-panel`/`#intro`の`getComputedStyle().display`が実際に`none`
